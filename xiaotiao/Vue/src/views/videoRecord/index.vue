		...
		<el-card shadow="never" class="glass-card mb-4">
			...
			<div class="table-wrapper">
				<el-table :data="state.tableData.data" v-loading="state.tableData.loading" class="custom-table">
					...
					<el-table-column label="原始视频" width="180" align="center">
						<template #default="scope">
							<div class="video-preview-box">
								<video class="table-video" :key="scope.row.inputVideo + uniqueKey">
									<source :src="scope.row.inputVideo" type="video/mp4" />
								</video>
								<div class="video-overlay"><el-icon><VideoPlay /></el-icon></div>
							</div>
						</template>
					</el-table-column>

					<el-table-column label="AI 处理结果" width="180" align="center">
						<template #default="scope">
							<div class="video-preview-box result-box">
								<video class="table-video" :key="scope.row.outVideo + uniqueKey">
									<source :src="scope.row.outVideo" type="video/mp4" />
								</video>
								<div class="video-overlay"><el-icon><Monitor /></el-icon></div>
							</div>
						</template>
					</el-table-column>
					...
					<el-table-column prop="kind" label="农作物种类">
						<template #default="scope">
							<el-tag size="small" effect="light" type="success">{{ scope.row.kind || '-' }}</el-tag>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" width="180" fixed="right" align="right">
						<template #default="scope">
							<el-button link type="primary" @click="show(scope.row)">对比分析</el-button>
...
import { Search, VideoPlay, Monitor } from '@element-plus/icons-vue';
...
const getTableData = () => {
	...
	request.get('/api/videoRecords', { params: state.tableData.param }).then((res) => {
		if (res.code == 0) {
...
const onRowDel = (row: any) => {
	ElMessageBox.confirm(`此操作将永久删除该视频记录，是否继续?`, '提示', {
...
.custom-table {
	...
	.video-preview-box {
		...
		&:hover {
			.video-overlay { opacity: 1; background: rgba(16, 185, 129, 0.3); }
			.table-video { opacity: 1; transform: scale(1.05); }
		}
	}
